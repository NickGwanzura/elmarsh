...
export function Footer() { return <footer className="footer">
  <div className="container footer-grid">
    ...
    <div><h3>Speak to our team</h3>{site.phones.map(p => <a key={p.name} href={p.href}><Phone size={16}/>{p.name}: {p.display}</a>)}<a href={`mailto:${site.email}`}>{site.email}</a><div className="socials"><a href="https://www.facebook.com/share/19S57GeAqY/?mibextid=wwXIfr" target="_blank" rel="noreferrer" aria-label="Elmarsh on Facebook"><Globe2/></a><a href="https://www.instagram.com/elmarsh_logistics?utm_source=qr" target="_blank" rel="noreferrer" aria-label="Elmarsh on Instagram"><Camera/></a><a href="#" aria-label="LinkedIn placeholder"><Network/></a><a href="https://www.tiktok.com/@elmarsh11" target="_blank" rel="noreferrer" aria-label="Elmarsh on TikTok"><Music2/></a></div></div>
  </div>
